...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("TERRA_TEST_LIVE") == "TRUE" ||
		os.Getenv("TERRA_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("TERRA_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.TerraSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.TerraError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.TerraSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.TerraError{Msg: msg, Code: code}
}
