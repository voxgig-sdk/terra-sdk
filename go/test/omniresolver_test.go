...
import (
	...
	omni "github.com/voxgig-sdk/terra-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.TerraError); is {
				out := map[string]any{
					"name":    "TerraError",
...
func omniCtx(arg any, client *sdk.TerraSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.TerraSDK); is {
			client = live
