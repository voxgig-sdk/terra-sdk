...
//
// NewTerraSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.TerraSDK {
	res, _ := kase["res"].([]any)
...
func fcCandidates(client *sdk.TerraSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.TerraSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.TerraSDK, name string) any {
