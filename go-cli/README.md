...
boru-driven command-line client **and** interactive REPL for the Terra
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/terra-cli)
make build
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/terra-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/terra-cli load 1 activity
   ```
...
```sh
./terra-cli load 1 activity          # scalar shorthand for {id:1}
./terra-cli load '{id:1}' activity     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `terra>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/terra-cli`. |
...
(`./terra-cli <expr>`) and interactively (the REPL), and expressions compose the
