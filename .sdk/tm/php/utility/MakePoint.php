...
// Terra SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class TerraMakePoint
{
    public static function call(TerraContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof TerraError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $select_def = TerraHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = TerraHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
