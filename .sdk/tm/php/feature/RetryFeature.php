...
class TerraRetryFeature extends TerraBaseFeature
{
    ...
    public function init(TerraContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (TerraContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(TerraContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(TerraContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
