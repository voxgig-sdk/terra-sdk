...
class TestBulkUserInfoEntity:

    def test_should_create_instance(self):
        testsdk = TerraSDK.test(None, None)
        ent = testsdk.BulkUserInfo(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _bulk_user_info_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "bulk_user_info." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set TERRA_TEST_BULK_USER_INFO_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        bulk_user_info_ref01_ent = client.BulkUserInfo(None)
...
def _bulk_user_info_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/bulk_user_info/BulkUserInfoTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "TERRA_TEST_BULK_USER_INFO_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("TERRA_TEST_BULK_USER_INFO_ENTID"))
