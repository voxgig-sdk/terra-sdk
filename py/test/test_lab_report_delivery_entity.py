...
class TestLabReportDeliveryEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.LabReportDelivery(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.LabReportDelivery(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set TERRA_TEST_LAB_REPORT_DELIVERY_ENTID JSON to run live")
        client = setup["client"]
