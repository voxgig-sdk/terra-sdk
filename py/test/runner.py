...
class TerraTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            TerraTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = TerraTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = TerraTestRunner.getenv("TERRA_TEST_LIVE")
        override = TerraTestRunner.getenv("TERRA_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = TerraTestRunner.getenv(key)
        ...
        explain = TerraTestRunner.getenv("TERRA_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["TERRA_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if TerraTestRunner._test_control is not None:
            return TerraTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                TerraTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            TerraTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = TerraTestRunner.load_test_control()
...
def load_env_local():
    TerraTestRunner.load_env_local()


def env_override(m):
    return TerraTestRunner.env_override(m)


def entity_data(v):
    return TerraTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return TerraTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return TerraTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return TerraTestRunner.load_test_control()


def live_client_options():
    return TerraTestRunner.live_client_options()


def live_delay_ms():
    return TerraTestRunner.live_delay_ms()
