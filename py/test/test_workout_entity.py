...
class TestWorkoutEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Workout(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Workout(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _workout_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "list", "load", "remove"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "workout." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set TERRA_TEST_WORKOUT_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        workout_ref01_ent = client.Workout(None)
        workout_ref01_data = helpers.to_map(vs.getprop(
            vs.getpath(setup["data"], "new.workout"), "workout_ref01"))

        workout_ref01_data = helpers.to_map(workout_ref01_ent.create(workout_ref01_data, None))
...
def _workout_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/workout/WorkoutTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("TERRA_TEST_WORKOUT_ENTID"))
