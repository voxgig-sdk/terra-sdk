...
class TestLabReportFileDirect:

    def test_should_direct_list_lab_report_file(self):
        setup = _lab_report_file_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-lab_report_file", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["lab_report_file01"]:
...
def _lab_report_file_direct_setup(mockres):
