...
class PlannedWorkoutEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return PlannedWorkoutEntity(self._client, opts)
    ...
    def data_get(self) -> PlannedWorkout:
    ...
    def match_get(self) -> PlannedWorkout:
    ...
    def load(self, reqmatch=None, ctrl=None) -> PlannedWorkout:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.PlannedWorkout().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[PlannedWorkout]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.PlannedWorkout().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
    ...
    def update(self, reqdata: PlannedWorkoutUpdateData, ctrl=None) -> PlannedWorkout:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
