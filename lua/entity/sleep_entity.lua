...
local SleepEntity = {}
SleepEntity.__index = SleepEntity


function SleepEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SleepEntity)
  self._name = "sleep"
  self._client = client
...
function SleepEntity:get_name()
  return self._name
end


function SleepEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SleepEntity.new(self._client, opts)
end
...
function SleepEntity:mark_deleted()
  self._deleted = true
end


function SleepEntity:deleted()
  return true == self._deleted
end


function SleepEntity:data_set(args)
  if args ~= nil then
...
function SleepEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SleepEntity:match_set(args)
  if args ~= nil then
...
function SleepEntity:match_get()
...
function SleepEntity:stream(action, args, callopts)
...
---@param reqmatch SleepLoadMatch
---@param ctrl? table
---@return Sleep
---@return string? err
function SleepEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function SleepEntity:_run_op(ctx, post_done)
  local utility = self._utility
