...
local DailyEntity = {}
DailyEntity.__index = DailyEntity


function DailyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DailyEntity)
  self._name = "daily"
  self._client = client
...
function DailyEntity:get_name()
  return self._name
end


function DailyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DailyEntity.new(self._client, opts)
end
...
function DailyEntity:mark_deleted()
  self._deleted = true
end


function DailyEntity:deleted()
  return true == self._deleted
end


function DailyEntity:data_set(args)
  if args ~= nil then
...
function DailyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DailyEntity:match_set(args)
  if args ~= nil then
...
function DailyEntity:match_get()
...
function DailyEntity:stream(action, args, callopts)
...
---@param reqmatch DailyLoadMatch
---@param ctrl? table
---@return Daily
---@return string? err
function DailyEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function DailyEntity:_run_op(ctx, post_done)
  local utility = self._utility
