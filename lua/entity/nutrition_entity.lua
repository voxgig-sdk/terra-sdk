...
local NutritionEntity = {}
NutritionEntity.__index = NutritionEntity


function NutritionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, NutritionEntity)
  self._name = "nutrition"
  self._client = client
...
function NutritionEntity:get_name()
  return self._name
end


function NutritionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return NutritionEntity.new(self._client, opts)
end
...
function NutritionEntity:mark_deleted()
  self._deleted = true
end


function NutritionEntity:deleted()
  return true == self._deleted
end


function NutritionEntity:data_set(args)
  if args ~= nil then
...
function NutritionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function NutritionEntity:match_set(args)
  if args ~= nil then
...
function NutritionEntity:match_get()
...
function NutritionEntity:stream(action, args, callopts)
...
---@param reqmatch NutritionLoadMatch
---@param ctrl? table
---@return Nutrition
---@return string? err
function NutritionEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function NutritionEntity:_run_op(ctx, post_done)
  local utility = self._utility
