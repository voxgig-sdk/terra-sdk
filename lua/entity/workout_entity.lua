...
local WorkoutEntity = {}
WorkoutEntity.__index = WorkoutEntity


function WorkoutEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WorkoutEntity)
  self._name = "workout"
  self._client = client
...
function WorkoutEntity:get_name()
  return self._name
end


function WorkoutEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WorkoutEntity.new(self._client, opts)
end
...
function WorkoutEntity:mark_deleted()
  self._deleted = true
end


function WorkoutEntity:deleted()
  return true == self._deleted
end


function WorkoutEntity:data_set(args)
  if args ~= nil then
...
function WorkoutEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WorkoutEntity:match_set(args)
  if args ~= nil then
...
function WorkoutEntity:match_get()
...
function WorkoutEntity:stream(action, args, callopts)
...
---@param reqmatch WorkoutLoadMatch
---@param ctrl? table
---@return Workout
---@return string? err
function WorkoutEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch WorkoutListMatch
---@param ctrl? table
---@return Workout[]
---@return string? err
function WorkoutEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata WorkoutCreateData
---@param ctrl? table
---@return Workout
---@return string? err
function WorkoutEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqmatch WorkoutRemoveMatch
---@param ctrl? table
---@return Workout
---@return string? err
function WorkoutEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function WorkoutEntity:_run_op(ctx, post_done)
  local utility = self._utility
