...
local AthleteEntity = {}
AthleteEntity.__index = AthleteEntity


function AthleteEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AthleteEntity)
  self._name = "athlete"
  self._client = client
...
function AthleteEntity:get_name()
  return self._name
end


function AthleteEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AthleteEntity.new(self._client, opts)
end
...
function AthleteEntity:mark_deleted()
  self._deleted = true
end


function AthleteEntity:deleted()
  return true == self._deleted
end


function AthleteEntity:data_set(args)
  if args ~= nil then
...
function AthleteEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AthleteEntity:match_set(args)
  if args ~= nil then
...
function AthleteEntity:match_get()
...
function AthleteEntity:stream(action, args, callopts)
...
---@param reqmatch AthleteLoadMatch
---@param ctrl? table
---@return Athlete
---@return string? err
function AthleteEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function AthleteEntity:_run_op(ctx, post_done)
  local utility = self._utility
