...
describe("PlannedWorkoutDirect", function()
  it("should direct-list-planned_workout", function()
    local setup = planned_workout_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-planned_workout", setup.live and "live" or "unit")
  ...
  it("should direct-load-planned_workout", function()
    local setup = planned_workout_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-planned_workout", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function planned_workout_direct_setup(mockres)
