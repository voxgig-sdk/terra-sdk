...
describe("LabReportDeliveryDirect", function()
  it("should direct-list-lab_report_delivery", function()
    local setup = lab_report_delivery_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-lab_report_delivery", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"lab_report_delivery01"}) do
...
function lab_report_delivery_direct_setup(mockres)
