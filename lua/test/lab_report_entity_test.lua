...
describe("LabReportEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:LabReport(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:LabReport(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:LabReport(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = lab_report_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "list", "load", "remove"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "lab_report." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set TERRA_TEST_LAB_REPORT_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local lab_report_ref01_ent = client:LabReport(nil)
    local lab_report_ref01_data = helpers.to_map(vs.getprop(
    ...
    local lab_report_ref01_list_result, err = lab_report_ref01_ent:list(lab_report_ref01_match, nil)
    assert.is_nil(err)
...
function lab_report_basic_setup(extra)
  runner.load_env_local()

  local entity_data_file = _test_dir .. "../../.sdk/test/entity/lab_report/LabReportTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read lab_report test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("TERRA_TEST_LAB_REPORT_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["TERRA_TEST_LAB_REPORT_ENTID"])
