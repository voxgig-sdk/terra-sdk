...
describe("WorkoutDirect", function()
  it("should direct-list-workout", function()
    local setup = workout_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-workout", setup.live and "live" or "unit")
  ...
  it("should direct-load-workout", function()
    local setup = workout_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-workout", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function workout_direct_setup(mockres)
