// Command Terra-mcp is an MCP server exposing the Terra
// SDK as MCP tools.
//
// Two tools are registered:
//
//	Terra_list  — list records of an entity
//	Terra_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: TERRA_APIKEY carries the API key and
	// TERRA_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("TERRA_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("TERRA_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewTerraSDK(opts)
