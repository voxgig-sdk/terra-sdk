...
[MCP](https://modelcontextprotocol.io) server exposing the Terra SDK as
two agent tools — `terra_list` and `terra_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/terra-mcp)
make build

# 2. Provide credentials via the environment
export TERRA_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user terra \
  -- /absolute/path/to/terra-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./terra-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// terra_list: first page of records
{ "entity": "integration" }
{ "entity": "integration", "query": { } }

// terra_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/terra-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user terra \
     -- "$PWD"/dist/*/terra-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `terra_list` and `terra_load` tools now appear
   in new sessions. Ask the agent to *"list integration using terra"*
   and it calls `terra_list` with `{"entity":"integration"}`.
...
```sh
export TERRA_APIKEY=sk_live_xxx            # API key
export TERRA_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `terra_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `terra_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `TERRA_APIKEY` | API key sent with every request. |
| `TERRA_BASE` | Optional override of the API base URL. |
...
```sh
./terra-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"terra_load","arguments":{"entity":"activity","query":{"id":1}}}}'
```
