...
class TerraNetsimFeature extends TerraBaseFeature
{
    ...
    private function _simulate(TerraContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(TerraContext $ctx, array $applied): void
    {
        $client = $this->client;
