...
class LabReportEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new LabReportEntity($this->_client, $opts);
    }

    /**
     * @param LabReport|array $args LabReport data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = TerraHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return LabReport|array The current LabReport data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of LabReport fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = TerraHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of LabReport fields).
     */
    ...
    /**
     * Load a single LabReport.
     *
     * @param LabReportLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed LabReportLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return LabReport|array The loaded LabReport as an assoc-array at the
     *   SDK boundary; throws TerraError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = TerraHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List LabReport items matching the given filter.
     *
     * @param LabReportListMatch|array|null $reqmatch Match filter (any subset
     *   of LabReport fields) as an assoc-array; LabReportListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return LabReport[]|array A list of LabReport items as assoc-arrays at
     *   the SDK boundary; throws TerraError on failure (item-5 convention).
     */
     ...
     *
     * @param LabReportCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed LabReportCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return LabReport|array The created LabReport as an assoc-array at the
     *   SDK boundary; throws TerraError on failure (item-5 convention).
     */
    ...
    /**
     * Remove an LabReport matching the given criteria.
     *
     * @param LabReportRemoveMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; LabReportRemoveMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return LabReport|array The removed LabReport as an assoc-array at the
     *   SDK boundary; throws TerraError on failure (item-5 convention).
     */
