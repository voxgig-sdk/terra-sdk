...
class FtCtrl extends TerraControl
{
...
class FtClient
{
    ...
    private TerraUtility $utility;

    public function __construct(TerraUtility $utility, array $options)
    {
    ...
    public function get_utility(): TerraUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TerraContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (TerraContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TerraConfig::make_feature($name);
    ...
    private static function buildUrl(TerraSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TerraError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TerraResponse($fetched);
        }
    ...
    private function fail(TerraContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TerraContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
