...
{
    private static function utility(): TerraUtility
    {
        return new TerraUtility();
    }
    ...
    private static function ctx(array $over = []): TerraContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new TerraContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new TerraOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof TerraError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new TerraOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = TerraMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new TerraOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = TerraMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new TerraOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = TerraMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TerraSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = TerraMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new TerraResponse([]),
            'result' => new TerraResult([]),
        ]);
        [, $err] = TerraMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new TerraSpec(['step' => 's']),
            'response' => new TerraResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new TerraResult([]),
        ]);
        [, $err] = TerraMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new TerraSpec(['step' => 's']),
            'response' => new TerraResponse(self::resp(200, ['v' => 1])),
            'result' => new TerraResult([]),
        ]);
        [, $err] = TerraMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new TerraSpec(['step' => 's']),
            'response' => new TerraResponse(self::resp(200, ['v' => 2])),
            'result' => new TerraResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        TerraMakeResponse::call($ctx);
    ...
    {
        $preset = new TerraResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new TerraSpec([]),
            'response' => new TerraResponse([]),
            'result' => new TerraResult([]),
        ...
        [$got, $err] = TerraMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new TerraResult([])]);
        [, $err] = TerraMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new TerraSpec([]), 'result' => null]);
        [, $err] = TerraMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TerraSpec(['step' => 's']),
            'result' => new TerraResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new TerraOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TerraMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TerraSpec(['step' => 's']),
            'result' => new TerraResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new TerraOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TerraMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TerraResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new TerraSpec([]),
            'result' => new TerraResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = TerraMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $utility = self::utility();
        $boom = new TerraError('boom', 'boom');
        $utility->fetcher = function (TerraContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TerraSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = TerraMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (TerraContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = TerraMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(TerraResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (TerraContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TerraSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = TerraMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new TerraResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new TerraSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = TerraMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = TerraMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = TerraMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new TerraResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, TerraDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new TerraResult(['ok' => false])]);
        $this->expectException(TerraError::class);
        TerraDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new TerraResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', TerraMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new TerraBaseFeature();
        $b = new TerraBaseFeature();
        TerraFeatureAdd::call($ctx, $a);
        TerraFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): TerraBaseFeature
    {
        $f = new TerraBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    private static function auth_ctx(array $options, ?array $headers): TerraContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new TerraSpec(['headers' => $headers]),
        ]);
    }
    ...
    {
        $ctx = self::ctx([
            'response' => new TerraResponse(['headers' => null]),
            'result' => new TerraResult([]),
        ]);
        TerraResultHeaders::call($ctx);
