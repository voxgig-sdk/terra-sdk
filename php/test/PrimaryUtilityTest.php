...
// Terra SDK primary utility test
//
...
//   TerraOmni::unwrap, which throws the err so omni can match it
...
//   builds the typed context with TerraOmni::ctx_from_map, runs the
//   utility, and writes the observable state back with
//   TerraOmni::sync_ctx BEFORE throwing any error - that map is
...
class PrimaryUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = TerraOmni::makeRunner(
                self::TEST_JSON_FILE, TerraSDK::test(null, null));
    ...
    private static function client(): TerraSDK
    {
    ...
    {
        $client = TerraSDK::test(null, null);
    ...
    public function test_done_basic(): void
    {
        ...
        $this->runsection('done', function (array $ctxmap) use ($client, $utility) {
            $ctx = TerraOmni::ctx_from_map($ctxmap, $client, $utility);
    ...
    public function test_make_error_basic(): void
    {
        ...
        $this->runsection('makeError', function ($ctxmap = null, $errmap = null) use ($client, $utility) {
            $ctx = TerraOmni::ctx_from_map(
                is_array($ctxmap) ? $ctxmap : [], $client, $utility);
            $err = TerraOmni::err_from_map(is_array($errmap) ? $errmap : null);
    ...
    {
        $hook_client = TerraSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends TerraBaseFeature {
            public $hook_fn = null;
            public function TestHook(TerraContext $ctx): void
            {
    ...
    {
        $init_client = TerraSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends TerraBaseFeature {
            public $init_fn = null;
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new TerraSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_url_basic(): void
    {
        ...
        $this->runsection('makeUrl', function (array $ctxmap) use ($client, $utility) {
            ...
            return TerraOmni::unwrap(($utility->make_url)($ctx));
        });
    }
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = TerraSDK::test(null, is_array($setup) ? $setup : null);
        ...
        $this->runsection('prepareAuth', function (array $ctxmap) use ($auth_client, $auth_utility, $utility) {
            $ctx = TerraOmni::ctx_from_map($ctxmap, $auth_client, $auth_utility);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runsection('resultBasic', function (array $ctxmap) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof TerraError)
