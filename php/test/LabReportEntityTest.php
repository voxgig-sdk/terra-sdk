...
class LabReportEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = TerraSDK::test(null, null);
        $ent = $testsdk->LabReport(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->LabReport(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->LabReport(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = lab_report_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "list", "load", "remove"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "lab_report." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set TERRA_TEST_LAB_REPORT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $lab_report_ref01_ent = $client->LabReport(null);
...
function lab_report_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/lab_report/LabReportTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["lab_report01", "lab_report02", "lab_report03"] as $k) {
    ...
    $entid_env_raw = getenv("TERRA_TEST_LAB_REPORT_ENTID");
