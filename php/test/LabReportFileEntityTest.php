...
class LabReportFileEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = TerraSDK::test(null, null);
        $ent = $testsdk->LabReportFile(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->LabReportFile(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->LabReportFile(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set TERRA_TEST_LAB_REPORT_FILE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $lab_report_file_ref01_data = null;
...
function lab_report_file_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/lab_report_file/LabReportFileTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["lab_report_file01", "lab_report_file02", "lab_report_file03", "session01"] as $k) {
    ...
    $entid_env_raw = getenv("TERRA_TEST_LAB_REPORT_FILE_ENTID");
