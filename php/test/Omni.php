...
//    only - a typed TerraContext there would make every ctx
...
final class TerraOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new TerraContext($clean, null);
    ...
    public static function err_from_map($m): ?TerraError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new TerraError(is_string($code) ? $code : '', $msg);
    }
