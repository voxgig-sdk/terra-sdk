...
(0, node_test_1.describe)('PlannedWorkoutDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-planned_workout', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-planned_workout', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-planned_workout', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-planned_workout', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.TerraSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
            apikey: env.TERRA_APIKEY,
        }));
        let idmap = env['TERRA_TEST_PLANNED_WORKOUT_ENTID'];
...
//# sourceMappingURL=PlannedWorkoutDirect.test.js.map