...
function isRunnable(code) {
    return (/new\s+TerraSDK\b/.test(code) ||
        /\bTerraSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+TerraSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/TerraSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
