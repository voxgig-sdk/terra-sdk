...
describe('DailyEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'daily.' + op, live)) return
    }
    ...
    let daily_ref01_data = Object.values(setup.data.existing.daily)[0] as any

    // LOAD
    const daily_ref01_ent = client.Daily()
    const daily_ref01_match_dt0: any = {}
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/daily/DailyTestData.json')
