...
describe('PlannedWorkoutDirect', async () => {
  ...
  test('direct-load-planned_workout', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-planned_workout', setup.live)) return
  ...
  test('direct-list-planned_workout', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-planned_workout', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['TERRA_TEST_PLANNED_WORKOUT_ENTID']
