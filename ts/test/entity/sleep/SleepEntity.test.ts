...
describe('SleepEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'sleep.' + op, live)) return
    }
    ...
    let sleep_ref01_data = Object.values(setup.data.existing.sleep)[0] as any

    // LOAD
    const sleep_ref01_ent = client.Sleep()
    const sleep_ref01_match_dt0: any = {}
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/sleep/SleepTestData.json')
  ...
  const idmapEnvVal = process.env['TERRA_TEST_SLEEP_ENTID']
