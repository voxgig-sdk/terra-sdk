...
describe('WorkoutDirect', async () => {
  ...
  test('direct-load-workout', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-workout', setup.live)) return
  ...
  test('direct-list-workout', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-workout', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['TERRA_TEST_WORKOUT_ENTID']
