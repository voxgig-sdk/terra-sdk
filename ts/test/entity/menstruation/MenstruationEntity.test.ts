...
describe('MenstruationEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'menstruation.' + op, live)) return
    }
    ...
    let menstruation_ref01_data = Object.values(setup.data.existing.menstruation)[0] as any

    // LOAD
    const menstruation_ref01_ent = client.Menstruation()
    const menstruation_ref01_match_dt0: any = {}
